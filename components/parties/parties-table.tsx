...
import { useParties, useDeleteParty, useDeletedParties, useHardDeleteParty } from "@/hooks/parties"
...
export function PartiesTable() {
  ...
  const { data: deletedData, isLoading: isLoadingDeleted } = useDeletedParties()

  const deleteParty = useDeleteParty()
  const hardDeleteParty = useHardDeleteParty()

  const parties = data?.parties || []
  const deletedParties = deletedData?.parties || []
  ...
  const handleHardDelete = (party: PoliticalParty) => {
    setSelectedParty(party)
  ...
  const confirmHardDelete = () => {
    if (selectedParty) {
      hardDeleteParty.mutate(selectedParty.id)
          ...
          {isLoadingDeleted ? (
            ...
            <div className="rounded-lg border bg-card">
              <div className="p-4 border-b bg-destructive/5">
                <div className="flex items-center gap-2 text-sm text-destructive">
                  ...
                  <span>These parties have been soft-deleted. You can permanently delete them to free up the party name for reuse.</span>
                </div>
              </div>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  <AnimatePresence>
                    {deletedParties.map((party, index) => (
                      <motion.tr
                        key={party.id}
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                      ...
                      >
                        ...
                        <TableCell>
                          {party.deleted_at ? (
                            <span className="text-sm text-muted-foreground">
                              {formatDistanceToNow(new Date(party.deleted_at), { addSuffix: true })}
                            </span>
                          ) : (
                            <span className="text-sm text-muted-foreground">-</span>
                          )}
                        ...
                        <TableCell className="text-right">
                          <Button
                            variant="destructive"
                            size="sm"
                            onClick={() => handleHardDelete(party)}
                            disabled={hardDeleteParty.isPending}
                          >
                            <Trash2 className="h-4 w-4 mr-2" />
                            Permanently Delete
                          </Button>
            ...
            <div className="rounded-lg border bg-card p-8">
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-muted-foreground">No deleted parties</p>
              </div>
            </div>
          )}
        </TabsContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                Are you sure you want to <strong>permanently delete</strong> "{selectedParty?.name}"?
              </p>
              <p className="text-destructive font-medium">
                This action is irreversible and will remove all data associated with this party.
              </p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
